...
-- 好像真的在翻译呢, 就是在翻译呢，都看到emit指令了
-- <op, dst, src, off, imm>
local function emit(op, dst, src, off, imm)
--	print("emit", op, dst, src, off, imm)
	io.write(string.format("emit: 0x%x %d %d %d ", op, dst, src, off))
	print(imm)
...
local function reg_alloc(var, reg)
	...
	if reg then
		for k,v in pairs(V) do -- Spill any variable that has this register
			if v.reg == reg and not v.shadow then --shadow是啥东东
				reg_spill(k)
				break
			end
		end
...
return setmetatable(BC, {
	...
	__call = function (t, op, a, b, c, d) ---强大的table用法, luajit
		print("call here ----->")
...
local function compile(prog, params)
	...
	print ("Before loadstring")	
	if type(prog) == 'string' then 
		print "Enter loadstring"
		prog = loadstring(prog) 
	end
	...
	print("before bytecode.decoder")
	-- 在这里会 生成一部分 的代码, 为啥会分开 要有一部分代码先走呢
	-- 或者这里的 代码本来 就不是一样的
	-- 
...
local function trace_bpf(ptype, pname, pdef, retprobe, prog, pid, cpu, group_fd)
	-- Load BPF program
	print("----- Before trace_bpf")
	if type(prog) ~= 'table' then
		prog = compile(prog, {proto.pt_regs}) -- add regs
	end
	print("----- End trace_bpf")
	print("prog:", prog) -- return a table here? -- 下载时出错
	local prog_fd, err, log = S.bpf_prog_load(S.c.BPF_PROG.KPROBE, prog.insn, prog.pc)
	--io.write(string.format("%d", prog_fd))
	assert(prog_fd, tostring(err)..': '..tostring(log))
	-- Open tracepoint and attach
